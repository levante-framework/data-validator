...
class LevanteRun(RunBase):
    ...
    _ALLOWED_STOP_REASONS = {"taskAbort", "timeOut", "errorOut", "sufficientTrials", "earlyCompletion", "experimenterExit"}

    @model_validator(mode='after')
    def check_stop_reason(self):
        if self.stop_reason is None:
            return self
        normalized = str(self.stop_reason).strip()
        if not normalized:
            return self
        if normalized not in self._ALLOWED_STOP_REASONS:
            if self.warning_msg_run:
                self.warning_msg_run = f"{self.warning_msg_run};stop_reason_invalid({normalized})"
            else:
                self.warning_msg_run = f"stop_reason_invalid({normalized})"
        return self
