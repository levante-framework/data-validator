...
| Field | Rule |
|-------|------|
| `language` | `en` → `en-US`, `es` → `es-CO`, `de` → `de-DE`; any other non-empty code → `Other(<original>)`; blank/`None` → `None` |
| `adaptive` | Set to `True` if `variant_name` contains `"adaptive"` (case-insensitive); otherwise `False` |
